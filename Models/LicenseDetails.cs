...
namespace LicenseServer.Models
{
    ...
    public class LicenseDetails
    {
        ...
        {
            int ifDbExist = 0;
            using (var connection = new SqlConnection(LicensedbConnectionStr))
            {
                var param = new { @databaseName = model.DatabaseName };
                var query = $"SELECT CASE WHEN EXISTS (SELECT 1 FROM sys.databases WHERE name = @databaseName) THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END AS DatabaseExists;";
                ifDbExist = await connection.ExecuteScalarAsync<int>(query, param);
            }
            if (ifDbExist != 1)
            {
                return 0;
